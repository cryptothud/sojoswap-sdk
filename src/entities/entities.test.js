...
describe('entities', () => {
    DECIMAL_PERMUTATIONS.forEach(decimals => {
        describe(`decimals permutation: ${decimals}`, () => {
            ...
            it('Pair', () => {
                pairs = [
                    new Pair(CurrencyAmount.fromRawAmount(tokens[0], decimalize(1, tokens[0].decimals)), CurrencyAmount.fromRawAmount(tokens[1], decimalize(1, tokens[1].decimals)), UNISWAP_DEFAULT_CONFIG),
                    new Pair(CurrencyAmount.fromRawAmount(tokens[1], decimalize(1, tokens[1].decimals)), CurrencyAmount.fromRawAmount(tokens[2], decimalize(1, tokens[2].decimals)), UNISWAP_DEFAULT_CONFIG),
                    new Pair(CurrencyAmount.fromRawAmount(tokens[2], decimalize(1, tokens[2].decimals)), CurrencyAmount.fromRawAmount(WETH9, decimalize(1234, WETH9.decimals)), UNISWAP_DEFAULT_CONFIG)
                ];
            });
            let route;
            it('Route', () => {
            ...
            it('#midPrice', () => {
                ...
                expect(route.midPrice.quote(CurrencyAmount.fromRawAmount(route.input, decimalize(1, route.input.decimals))).toExact()).toEqual(CurrencyAmount.fromRawAmount(route.output, decimalize(1234, route.output.decimals)).toExact());
                expect(route.midPrice
                    .invert()
                    .quote(CurrencyAmount.fromRawAmount(route.output, decimalize(1234, route.output.decimals)))
                    .toExact()).toEqual(CurrencyAmount.fromRawAmount(route.input, decimalize(1, route.input.decimals)).toExact());
            ...
            describe('Trade', () => {
                ...
                it('TradeType.EXACT_INPUT', () => {
                    route = new Route([
                        new Pair(CurrencyAmount.fromRawAmount(tokens[1], decimalize(5, tokens[1].decimals)), CurrencyAmount.fromRawAmount(WETH9, decimalize(10, WETH9.decimals)), UNISWAP_DEFAULT_CONFIG)
                    ], tokens[1], WETH9);
                    ...
                    const trade = new Trade(route, inputAmount, TradeType.EXACT_INPUT, UNISWAP_DEFAULT_CONFIG);
                ...
                it('TradeType.EXACT_OUTPUT', () => {
                    ...
                    const trade = new Trade(route, outputAmount, TradeType.EXACT_OUTPUT, UNISWAP_DEFAULT_CONFIG);
                    expect(trade.route).toEqual(route);
                ...
                it('minimum TradeType.EXACT_INPUT', () => {
                    if ([9, 18].includes(tokens[1].decimals)) {
                        const route = new Route([
                            new Pair(CurrencyAmount.fromRawAmount(tokens[1], decimalize(1, tokens[1].decimals)), CurrencyAmount.fromRawAmount(WETH9, JSBI.add(decimalize(10, WETH9.decimals), tokens[1].decimals === 9 ? JSBI.BigInt('30090280812437312') : JSBI.BigInt('30090270812437322'))), UNISWAP_DEFAULT_CONFIG)
                        ], tokens[1], WETH9);
                        const outputAmount = CurrencyAmount.fromRawAmount(tokens[1], '1');
                        const trade = new Trade(route, outputAmount, TradeType.EXACT_INPUT, UNISWAP_DEFAULT_CONFIG);
