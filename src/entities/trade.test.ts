...
describe('Trade', () => {
  ...
  describe('#bestTradeExactIn', () => {
    it('throws with empty pairs', () => {
      expect(() => Trade.bestTradeExactIn([], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token2, UNISWAP_DEFAULT_CONFIG)).toThrow(
        'PAIRS'
      )
    })
    it('throws with max hops of 0', () => {
      expect(() =>
        Trade.bestTradeExactIn([pair_0_2], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token2, UNISWAP_DEFAULT_CONFIG, {
          maxHops: 0
        })
      ).toThrow('MAX_HOPS')
    })
    ...
    it('doesnt throw for zero liquidity pairs', () => {
      expect(
        Trade.bestTradeExactIn([empty_pair_0_1], CurrencyAmount.fromRawAmount(token0, JSBI.BigInt(100)), token1, UNISWAP_DEFAULT_CONFIG)
      ).toHaveLength(0)
    })
  ...
  describe('#bestTradeExactOut', () => {
    it('throws with empty pairs', () => {
      expect(() => Trade.bestTradeExactOut([], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100)), UNISWAP_DEFAULT_CONFIG)).toThrow(
        'PAIRS'
      )
    })
    it('throws with max hops of 0', () => {
      expect(() =>
        Trade.bestTradeExactOut([pair_0_2], token0, CurrencyAmount.fromRawAmount(token2, JSBI.BigInt(100)), UNISWAP_DEFAULT_CONFIG, {
          maxHops: 0
        })
      ).toThrow('MAX_HOPS')
    })
    ...
    it('doesnt throw for zero liquidity pairs', () => {
      expect(
        Trade.bestTradeExactOut([empty_pair_0_1], token1, CurrencyAmount.fromRawAmount(token1, JSBI.BigInt(100)), UNISWAP_DEFAULT_CONFIG)
      ).toHaveLength(0)
    })
